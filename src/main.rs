...
use cards::{get_shuffled_cards, DEFAULT_SEQUENCE, get_highest_card};
...
fn main() {
    ...
    loop {
        ...
        let player_played_card = handle_play_card(&mut player_cards, &flipped_card, &DEFAULT_SEQUENCE, &current_game_state);

        let current_hand = vec![bot_1_played_card, parther_played_card, bot_2_played_card, player_played_card.clone()];
